                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <form action="{{route('ajoutVehicule')}}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control" id="KM_defaut" name="KM_defaut" required>
                                        </div>
                                        <div class="form-group">
                            ...
                            <table class="table table-hover" id="dataTables-example" width="100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($vehicule->image)
                                                <img src="{{ asset('images/' . $vehicule->image) }}" alt="Image du véhicule" style="max-width: 100px; max-height: 100px;">
                                            @else
                                                Pas d'image
                                            @endif
                                        </td>
                                        ...
                                        <td colspan="2">
                                            ...
                                            <button class="btn btn-primary edit-btn" data-toggle="modal" data-target="#editModal" data-id="{{ $vehicule->id }}" data-marque="{{ $vehicule->marque }}" data-date="{{ $vehicule->date_achat }}" data-km="{{ $vehicule->KM_defaut }}" data-matricule="{{ $vehicule->matricule }}" data-image="{{ asset('images/' . $vehicule->image) }}">Modifier</button>
                                              ...
                                              <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <form id="editForm" action="{{ route('updateVehicule', $vehicule->id) }}" method="POST" enctype="multipart/form-data">
                                                            @csrf
                                                            @method('PUT')
                                                            <div class="modal-body">
                                                                ...
                                                                <div class="form-group">
                                                                    <label for="editKm">Kilométrage par défaut</label>
                                                                    <input type="number" class="form-control" id="editKm" name="editKm" required>
                                                                </div>
                                                                <div class="form-group">
                                                                    <label for="editMatricule">Matricule</label>
                                                                    <input type="text" class="form-control" id="editMatricule" name="editMatricule" required>
                                                                </div>
                                                                <div class="form-group">
                                                                    <label for="editImage">Image</label>
                                                                    <input type="file" class="form-control-file" id="editImage" name="editImage">
                                                                    <img id="editImagePreview" src="#" alt="Aperçu de l'image" style="max-width: 100px; max-height: 100px;">
                                                                </div>
                                                                <div class="form-group">
                                                                    <label for="editCategorie">Catégorie</label>
                                                                    <select class="form-control" id="editCategorie" name="editCategorie" required>
                                                                        @foreach($categories as $categorie)
                                                                            <option value="{{ $categorie->id }}">{{ $categorie->nomCat }}</option>
                                                                        @endforeach
                                                                    </select>
                                            ...
                                            <form action="{{ route('deleteVehicule', $vehicule->id) }}" method="POST" style="display: inline-block;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="fas fa-trash-alt" style="color: red;" onclick="return confirm('Êtes-vous sûr de vouloir supprimer ce véhicule ?')"></buttton>
                                            </form>
                                        </td>
                                    </tr>
                                    @endforeach
